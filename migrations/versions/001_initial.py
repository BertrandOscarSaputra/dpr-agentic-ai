...
def upgrade() -> None:
    ...
    op.create_table(
        "item_analysis",
        sa.Column("id", sa.Integer, primary_key=True),
        sa.Column("item_id", sa.Integer, sa.ForeignKey("content_items.id"), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column("created_at", sa.DateTime, server_default=sa.func.now()),
        sa.UniqueConstraint("item_id", "akd_name", name="uq_akd_mapping_item_akd"),
    )
    ...
    op.create_index("ix_item_analysis_sentiment", "item_analysis", ["sentiment"])
    op.create_index("ix_akd_mapping_akd_name", "akd_mapping", ["akd_name"])
    op.create_index("ix_trend_windows_anomaly", "trend_windows", ["is_anomaly"])
